...
export default function Trainers() {
  ...
  const next = () => {
    setCurrentIndex((prev) => (prev + 1) % (trainers.length - 1));
  };

  const prev = () => {
    setCurrentIndex((prev) => (prev - 1 + (trainers.length - 1)) % (trainers.length - 1));
  };
...
export function TrainerCard({ trainer, index }: { trainer: typeof trainers[0], index: number }) {
  ...
  return (
    <motion.div 
      ...
      className="bg-card rounded-2xl overflow-hidden border border-white/5 flex flex-col group h-full"
    >
      <div className="relative h-80 w-full overflow-hidden">
        ...
        <div className="absolute top-4 right-4 bg-accent p-2 rounded-tl-xl rounded-br-xl rounded-tr-sm rounded-bl-sm text-black z-20">
